(() => {
  ...
  function saveReceiptPdf(bundle){
    ...
    const lines=doc.splitTextToSize(receiptText(bundle),w);
    doc.text(lines,m,43);
  ...
  async function copyReceipt(bundle){
    const text=receiptText(bundle);
    await navigator.clipboard.writeText(text);
    alert('Recibo copiado. Você pode enviar pelo WhatsApp ao Creator.');
  }

  async function deletePayment(id){
    if(!confirm('Apagar este registro de pagamento? Esta ação remove o registro financeiro desta tela.'))return;
    await rest(`creator_ads_payments?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}});
    await loadFinance();
  }

  async function deleteCreator(id,card){
    const [subs,pays]=await Promise.all([
      rest(`creator_ads_submissions?profile_id=eq.${encodeURIComponent(id)}&select=id&limit=1`),
      rest(`creator_ads_payments?profile_id=eq.${encodeURIComponent(id)}&select=id&limit=1`)
    ]);
    if((subs&&subs.length)||(pays&&pays.length)){
      alert('Este Creator já possui histórico de vídeo ou pagamento. Para preservar os registros, use “Desativar Creator” em vez de apagar.');
      return;
    }
    if(!confirm('Apagar definitivamente este cadastro de Creator?'))return;
    await rest(`creator_ads_profiles?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}});
    card?.remove();
  }
  ...
  function enhanceCreators(){
    document.querySelectorAll('#profilesList .item-card').forEach(card=>{
      ...
      const src=card.querySelector('[data-profile]');
      const id=src?.dataset.profile;
      ...
      b.addEventListener('click',e=>{e.stopPropagation();deleteCreator(id,card).catch(err=>alert(err.message));});
      actions.appendChild(b);
    });
  }

  document.addEventListener('click',async e=>{
    const tab=e.target.closest('[data-tab="pagamentos"]');if(tab){setTimeout(()=>loadFinance().catch(err=>alert(err.message)),100);return;}
  ...
  const obs=new MutationObserver(()=>{enhanceCreators();if(document.querySelector('[data-view="pagamentos"].active'))loadFinance().catch(()=>{});});
  obs.observe(document.documentElement,{subtree:true,childList:true});
  setTimeout(enhanceCreators,350);
})();