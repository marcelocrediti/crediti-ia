(() => {
  ...
  async function rpc(name, body) {
    ...
    if (!r.ok) throw new Error(text || 'Não foi possível concluir agora.');
...
#${OVERLAY_ID} .creator-ca-mark{width:44px;height:44px;border-radius:50%;display:grid;place-items:center;background:#ff0f73;color:#fff;font-size:18px;font-weight:900;letter-spacing:-.04em;margin-bottom:11px}
...
#${OVERLAY_ID} .creator-hero-note{margin-top:13px;padding:9px 11px;border:1px solid rgba(253,202,1,.35);border-radius:12px;background:rgba(253,202,1,.10);color:#fff;font-size:11px;line-height:1.25;font-weight:800}
...
#${UPLOAD_ID}{margin:0 0 12px;padding:14px;border:1px solid #deded9;border-radius:16px;background:#fff}
#${UPLOAD_ID} h3{margin:0 0 5px;font-size:16px}
#${UPLOAD_ID} p{margin:0 0 10px;color:#555;font-size:12px;line-height:1.45}
#${UPLOAD_ID} .upload-progress{margin-top:9px;font-size:12px;font-weight:800;line-height:1.4}
#${CARD_ID} .creator-ca-card{width:40px;height:40px;border-radius:50%;display:grid;place-items:center;background:#ff0f73;color:#fff;font-size:16px;font-weight:900;letter-spacing:-.04em;margin-bottom:2px}
@media(max-width:520px){#${OVERLAY_ID} .tabs{grid-template-columns:repeat(2,minmax(0,1fr))}#${OVERLAY_ID} .tabs button:last-child{grid-column:span 2}#${OVERLAY_ID} .creator-hero{grid-template-columns:minmax(0,1.22fr) minmax(112px,.78fr);min-height:225px}#${OVERLAY_ID} .creator-hero-copy{padding:17px 0 17px 16px}#${OVERLAY_ID} .creator-hero h3{font-size:19px;max-width:230px}#${OVERLAY_ID} .creator-hero-sub{font-size:11px;max-width:225px}#${OVERLAY_ID} .creator-hero-media,#${OVERLAY_ID} .creator-hero-media img{min-height:225px}#${OVERLAY_ID} .creator-steps{grid-template-columns:1fr}#${OVERLAY_ID} .creator-step{display:grid;grid-template-columns:34px 1fr;column-gap:9px;align-items:center;padding:10px 12px}#${OVERLAY_ID} .creator-step-icon{margin:0;grid-row:1/3}}
@media(max-width:390px){#${OVERLAY_ID} .creator-hero{grid-template-columns:1fr}#${OVERLAY_ID} .creator-hero-media{display:none}}
`;
    document.head.appendChild(style);
  }

  function applyCardIcon() {
    const card = document.getElementById(CARD_ID);
    if (!card || card.querySelector('.creator-ca-card')) return;
    const mark = document.createElement('span');
    mark.className = 'creator-ca-card';
    mark.textContent = 'CA';
  ...
  async function handleUpload(form) {
    ...
    if (!file) return;
    if (file.size > 157286400) { progress.textContent = 'Arquivo acima de 150 MB.'; return; }
    let begin = null;
    try {
      progress.textContent = 'Pendente: preparando envio...';
      const meta = await videoMeta(file);
      begin = await rpc('creator_ads_begin_upload', {
        p_token: token(), p_campaign: form.campaign.value, p_video_nome: file.name,
        p_video_mime: file.type || 'video/mp4', p_video_tamanho: file.size,
        p_video_largura: meta.width, p_video_altura: meta.height, p_video_duracao: meta.duration,
      });
      progress.textContent = 'Pendente: enviando arquivo original...';
      const upload = await fetch(`${STORAGE}/creator-ads-videos/${begin.path.split('/').map(encodeURIComponent).join('/')}`, {
      ...
      progress.textContent = 'Enviado: vídeo recebido com sucesso.';
      form.reset();
    } catch (error) {
      progress.textContent = `Não enviado: ${error.message}`;
      if (begin) {
        try { await rpc('creator_ads_finish_upload', { p_token:token(), p_submission:begin.submission_id, p_upload_token:begin.upload_token, p_video_path:begin.path, p_ok:false, p_error:error.message }); } catch {}
      }
    }
  }

  async function mountActiveUploader() {
    const videosBox = document.getElementById('creatorVideos');
    if (!videosBox || !token() || document.getElementById(UPLOAD_ID)) return;
    let me;
    try { me = await rpc('creator_ads_get_me', { p_token:token() }); } catch { return; }
    if (!me?.id || me.status !== 'ativo') return;
    let campaigns = [];
    ...
    block.id = UPLOAD_ID;
    if (!campaigns.length) {
      block.innerHTML = '<h3>Enviar vídeo</h3><p>Seu Creator está ativo. No momento não há campanha aberta para receber vídeo.</p>';
      videosBox.prepend(block);
      return;
    }
    block.innerHTML = `<h3>Enviar vídeo</h3><p>Escolha a campanha e envie o vídeo original em MP4, MOV ou WebM. Recomendado: 1080 × 1920, vertical 9:16.</p><form id="creatorActiveVideoForm"><label>Campanha<select name="campaign" required><option value="">Escolha a campanha</option>${campaigns.map(c=>`<option value="${esc(c.id)}">${esc(c.titulo)}</option>`).join('')}</select></label><label>Vídeo<input type="file" name="video" accept="video/mp4,video/quicktime,video/webm" required></label><label class="check"><input type="checkbox" required><span>Confirmo que o vídeo foi produzido por mim.</span></label><label class="check"><input type="checkbox" required><span>Confirmo as autorizações das pessoas e elementos do vídeo.</span></label><label class="check"><input type="checkbox" required><span>Confirmo que não utilizei conteúdo protegido sem autorização.</span></label><label class="check"><input type="checkbox" required><span>Li e aceito as regras da campanha escolhida.</span></label><button class="primary" type="submit">Enviar vídeo</button><div class="upload-progress" aria-live="polite"></div></form>`;
    videosBox.prepend(block);
    block.querySelector('form')?.addEventListener('submit', async (e) => { e.preventDefault(); await handleUpload(e.currentTarget); });
  }
  ...
  function preserveTabScroll(overlay) {
    ...
    overlay.querySelector('.tabs')?.addEventListener('click', (event) => {
      ...
      if (button.dataset.tab === 'videos') window.setTimeout(mountActiveUploader, 120);
    }, true);
  }
  ...
  function mountHero() {
    ...
    if (!hero) {
      ...
      hero.innerHTML = `<div class="creator-hero-copy"><span class="creator-ca-mark">CA</span><span class="creator-hero-badge">Creator Ads Crediti</span><h3>Crie conteúdo e fature com a Crediti</h3><p class="creator-hero-sub">Participe das campanhas, envie seu vídeo e tenha a chance de ser o creator escolhido da vez.</p><div class="creator-hero-note">Sua ideia pode virar campanha</div></div><div class="creator-hero-media"><img src="${PHOTO}" alt="Pessoa criando conteúdo com um smartphone" loading="lazy" referrerpolicy="no-referrer"></div>`;
    ...
    if (!steps) {
      ...
      steps.innerHTML = `<article class="creator-step"><span class="creator-step-icon">1</span><strong>Campanhas</strong><span>Veja o tema e o prêmio de cada oportunidade.</span></article><article class="creator-step"><span class="creator-step-icon">2</span><strong>Criação</strong><span>Grave seu vídeo no formato pedido.</span></article><article class="creator-step"><span class="creator-step-icon">3</span><strong>Envio</strong><span>Envie e acompanhe o resultado pelo app.</span></article>`;
  ...
  if (!mountHero()) {
    ...
    const observer = new MutationObserver(() => { applyCardIcon(); mountActiveUploader(); });
    observer.observe(document.documentElement, { childList:true, subtree:true });
  }
})();
