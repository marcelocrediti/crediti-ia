...
#${MAIN_ID}{
  ...
  padding:24px 122px 24px 24px!important;
  margin:18px 0 6px!important;
  background:linear-gradient(135deg,#087CFF 0%,#3157FF 48%,#D91FEA 100%)!important;
  color:#fff!important;
  box-shadow:0 14px 34px rgba(65,63,210,.20)!important;
...
#${MINI_ID}{
  ...
  border-radius:18px;
  padding:14px 16px;
  background:linear-gradient(135deg,#087CFF 0%,#5753FF 55%,#E21EDB 100%);
  color:#fff;
  font:inherit;
  text-align:left;
  display:grid;
  ...
  box-shadow:0 9px 24px rgba(80,70,210,.16);
}
#${MINI_ID} .copy{min-width:0}
#${MINI_ID} strong{display:block;font-size:15px;line-height:1.2;font-weight:900;margin-bottom:3px}
#${MINI_ID} small{display:block;color:rgba(255,255,255,.84);font-size:11px;line-height:1.35}
#${MINI_ID} .cta{color:#FDCA01;font-size:12px;font-weight:900;white-space:nowrap}
@media(max-width:420px){
  #${MAIN_ID}{min-height:195px!important;padding:21px 98px 21px 20px!important}
  #${MAIN_ID}:before{right:16px;width:62px;height:62px;border-radius:18px;font-size:29px}
  #${MAIN_ID} strong{font-size:24px!important}
  #${MAIN_ID} small{font-size:13px!important}
  #${MINI_ID}{width:calc(100% - 24px);margin-left:12px;margin-right:12px;padding:13px 14px}
}
`;
  ...
  function mountMini(label) {
    if (!['Crédito','Aprenda','Serviços'].includes(label)) {
      removeMini();
      return;
    }
    ...
    if (heading?.parentElement) {
      ...
      if (mini.parentElement !== parent || mini.previousElementSibling !== heading) heading.insertAdjacentElement('afterend', mini);
      return;
    }
    ...
    if (firstVisibleSection) {
      firstVisibleSection.appendChild(mini);
    } else if (root) {
  ...
  function sync() {
    ...
    if (homeVisible || active === 'Início' || active === 'Shop') {
      removeMini();
    } else {
      mountMini(active);
    }
